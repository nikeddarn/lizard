...
class WarrantyContentController extends Controller
{
    /**
     * @var string
     */
    const WARRANTY_PAGE_ROUTE_NAME = 'shop.warranty.index';
    /**
     * @var StaticPage
     */
    private $staticPage;
    ...
    public function edit()
    {
        ...
        $pageData = $this->staticPage->newQuery()->where('route', self::WARRANTY_PAGE_ROUTE_NAME)
            ->first();

        return view('content.admin.page_content.warranty.index')->with(compact('pageData'));
    }
    ...
    public function update(UpdateStaticPageDataRequest $request)
    {
        ...
        $staticPage = $this->staticPage->newQuery()->where('route', self::WARRANTY_PAGE_ROUTE_NAME)->first();

        if ($staticPage){
            $staticPage->update($pageData);
        }else{
            $pageData['route'] = self::WARRANTY_PAGE_ROUTE_NAME;
    ...
    public function uploadImage(Request $request)
    {
        ...
        return '/storage/' . $request->image->store('images/content/warranty', 'public');
    }
}
