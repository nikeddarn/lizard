...
class PaymentsContentController extends Controller
{
    /**
     * @var string
     */
    ...
    public function edit()
    {
        ...
        $pageData = $this->staticPage->newQuery()->where('route', self::PAYMENTS_PAGE_ROUTE_NAME)
            ->first();

        return view('content.admin.page_content.payments.index')->with(compact('pageData'));
    }
    ...
    public function update(UpdateStaticPageDataRequest $request)
    {
        ...
        $staticPage = $this->staticPage->newQuery()->where('route', self::PAYMENTS_PAGE_ROUTE_NAME)->first();

        if ($staticPage){
            $staticPage->update($pageData);
        }else{
            $pageData['route'] = self::PAYMENTS_PAGE_ROUTE_NAME;
    ...
    public function uploadImage(Request $request)
    {
        ...
        return '/storage/' . $request->image->store('images/content/payments', 'public');
    }
}
