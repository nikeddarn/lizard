...
class ReturnContentController extends Controller
{
    ...
    public function edit()
    {
        ...
        $pageData = $this->staticPage->newQuery()->where('route', self::RETURN_PAGE_ROUTE_NAME)
            ->first();

        return view('content.admin.page_content.return.index')->with(compact('pageData'));
    }
    ...
    public function update(UpdateStaticPageDataRequest $request)
    {
        ...
        $staticPage = $this->staticPage->newQuery()->where('route', self::RETURN_PAGE_ROUTE_NAME)->first();

        if ($staticPage){
            $staticPage->update($pageData);
        }else{
            $pageData['route'] = self::RETURN_PAGE_ROUTE_NAME;
    ...
    public function uploadImage(Request $request)
    {
        ...
        return '/storage/' . $request->image->store('images/content/return', 'public');
    }
}
